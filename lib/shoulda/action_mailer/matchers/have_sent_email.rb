      ...
      class HaveSentEmailMatcher # :nodoc:
        ...
        def matches?(subject)
          ::ActionMailer::Base.deliveries.each do |mail|
            ...
            @headers_failed = @headers.reject {|key, value| regexp_or_string_match(mail[key].to_s, value) }.any? if @headers
        ...
        def expectation
          ...
          expectation << " with headers #{@headers.inspect}" if @headers_failed
          expectation << "\nDeliveries:\n#{inspect_deliveries}"
        end
        ...
        def anything_failed?
          @subject_failed || @body_failed || @sender_failed || @recipient_failed || @headers_failed
        end
